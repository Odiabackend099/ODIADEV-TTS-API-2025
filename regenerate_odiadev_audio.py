"""
ODIADEV TTS Audio Regeneration Script
====================================

This script automatically regenerates all ODIADEV company audio content 
using the real OpenAI TTS API and saves them to the specified directory.

Target Directory: C:\\Users\\OD~IA\\Music\\ODIA-TTS-TEST-AUDIO
"""
...
def start_production_tts_server():
    """Start the production TTS server"""
    import subprocess
    import time
    
    print("🚀 Starting production TTS server...")
    
    try:
        # Start the production TTS server in background
        process = subprocess.Popen([
            sys.executable, "production_tts_app.py"
...
def generate_additional_voice_variants():
    """Generate key messages with different voices for variety"""
    return [
        {
...
def create_playlist_file(results):
    ...
    try:
        with open(playlist_file, 'w', encoding='utf-8') as f:
            f.write("ODIADEV Company Audio - Complete Playlist\\n")
...
def main():
    ...
    # Step 2: Start production TTS server
    server_process = start_production_tts_server()
    if not server_process:
        print("❌ Cannot proceed without TTS server")
        return False
    
    try:
        # Step 3: Get all audio segments to generate
        main_segments = get_odiadev_company_segments()
        voice_variants = generate_additional_voice_variants()
        all_segments = main_segments + voice_variants
        
        print(f"🎯 Generating {len(all_segments)} audio files...")
        print()
        ...
        if successful == len(all_segments):
            ...
            print("   Your ODIADEV company audio is ready for use!")
        elif successful > 0:
        ...
        # Step 7: Clean up server process
        try:
            if server_process:
                server_process.terminate()
                print("\\n🔧 Production TTS server stopped")
        except:
            pass
